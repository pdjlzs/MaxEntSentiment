...
int Labeler::createAlphabet(const vector<Instance>& vecInsts) {
  ...
  for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
    ...
    m_labelAlphabet.from_string(labels);
...
void Labeler::extractLinearFeatures(vector<string>& features, const Instance* pInstance) {
  ...
  for (int idx = 0; idx < words.size(); idx++) {
    features.push_back("Uni=" + words[idx]);
    features.push_back("Bi=" + (idx > 0 ? words[idx - 1] : "NULL") + "###" + words[idx]);
    if (idx == 0)
      features.push_back("Tri=NULL###NULL" + words[idx]);
    else if (idx == 1)
      features.push_back("Tri=NULL###" + words[idx - 1] + words[idx]);
    else
      features.push_back("Tri=" + words[idx - 2] + "###" + words[idx - 1] + words[idx]);
  }
}
